...
export async function POST(req: Request) {
  try {
    ...
    if (result) {
      return NextResponse.json({ success: true, message: "Buy executed successfully" });
    } else {
      return NextResponse.json({ success: false, message: "Swap execution failed" });
    }
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: "Failed to execute buy" }, { status: 500 });
  }
}
